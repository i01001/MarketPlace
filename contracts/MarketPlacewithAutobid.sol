...
contract MarketPlacewithAutobid is Ownable, ReentrancyGuard, ERC1155Holder {
    ...
    uint public AuctionListingPrice = 10**15;
    uint public listsalecomissionpercent = 5;
...
// Add auto bid option upto a limit - new contract
    ...
    function createItem(bool _nftType, string memory _tokenURI, uint _amountNFT1155) public returns (bytes memory){
        ...
        (bool success, bytes memory _tokenID) = NFT721Contract.call(abi.encodeWithSignature("mint(address,string)", msg.sender, _tokenURI));
        require(success);
        return _tokenID;
        }
        else{
        require (NFT1155Contract != address(0), "Set the NFT1155Contract Address via the function!");
        (bool success, bytes memory _tokenID) = NFT1155Contract.call(abi.encodeWithSignature("mint(address,string,uint256)", msg.sender, _tokenURI, _amountNFT1155));
        require(success);
        return _tokenID;
        }      
    } 
    ...
    function listItem (address _nftContract, uint _tokenID, bool _nftType, uint _amountNFT1155, bytes memory _data, bool _currency, uint _price) public payable returns(uint){
        ...
        if(_nftType == false){
        (bool success, ) = _nftContract.call(abi.encodeWithSignature("transferFrom(address,address,uint256)", payable(msg.sender), payable(address(this)), _tokenID));
        require(success, "Error transferring, ensure that this contract has been approved!");
        }
        else{
        (bool success, ) = _nftContract.call(abi.encodeWithSignature("safeTransferFrom(address,address,uint256,uint256,bytes)", msg.sender, address(this), _tokenID, _amountNFT1155, _data));
        require(success);
        }
        _counterListSale.increment();
        uint _currentItem = _counterListSale.current();
    ...
    function buyItem (uint _listingID) public payable returns (bool){
        ...
        require (_listingID <= _counterListSale.current(), "Invalid Listing ID!");
        if(listingIDtoItems[_listingID].nftType == false){
        (bool success, ) = (listingIDtoItems[_listingID].nftContract).call(abi.encodeWithSignature("transferFrom(address,address,uint256)", payable(address(this)), payable(msg.sender) , (listingIDtoItems[_listingID].tokenID)));
        require(success);
        }
        else{
        (bool success, ) = (listingIDtoItems[_listingID].nftContract).call(abi.encodeWithSignature("safeTransferFrom(address,address,uint256,uint256,bytes)", address(this), msg.sender, (listingIDtoItems[_listingID].tokenID), (listingIDtoItems[_listingID].amountNFT1155), (listingIDtoItems[_listingID].data)));
        require(success);
        }
        address payable _newadd = payable(address(msg.sender));
        uint _comm = (listingIDtoItems[_listingID].price)*listsalecomissionpercent/100;
        treasury += _comm;
   ...
   function AuctionSaleComission (uint _comission) public onlyOwner {
    ...
    function listItemOnAuction (address _nftContract, uint _tokenID, bool _nftType, bool _currency, uint _startprice, uint _amountNFT1155, bytes memory _data) public payable returns (uint){
        ...
        require(_startprice > 0, "Starting Bidding Price needs to be at least 1 wei");
        if(_nftType == false){
        (bool success, ) = _nftContract.call(abi.encodeWithSignature("transferFrom(address,address,uint256)", payable(msg.sender), payable(address(this)), _tokenID));
        require(success);
        }
        ...
        uint _currentItem = _counterListAuction.current();
   ...
   function makeBid (uint _auctionID) public payable returns (bool) {
        ...
        uint _lim = _value*101/100;
        if(auctionIDtoItems[_auctionID].Autobidder == address(0)){
            biddingcalc(payable(msg.sender), _auctionID, _value);
        }
        else{
            uint _current = auctionIDtoItems[_auctionID].autobidderlimit;
            if(_current < _value){
                biddingcalc(payable(msg.sender), _auctionID, _value);
            }
            else if(_lim < _current){
                biddingcalc(auctionIDtoItems[_auctionID].Autobidder, _auctionID, _lim);
                address payable _newadd = payable(address(msg.sender));
                payable(_newadd).transfer(_value);
            }
            else{
                biddingcalc(auctionIDtoItems[_auctionID].Autobidder, _auctionID, _current);
                address payable _newadd = payable(address(msg.sender));
                payable(_newadd).transfer(_value);
            }
        }
...
function biddingcalc (address payable _bidder, uint _auctionID, uint _value) private returns (bool){
        ...
        else if((auctionIDtoItems[_auctionID].numberofbids >= 1) && (auctionIDtoItems[_auctionID].Autobidder != address(0))){
            if((auctionIDtoItems[_auctionID].lastBidder == _bidder) && (auctionIDtoItems[_auctionID].Autobidder == _bidder)){                
            }
        ...
        if(auctionIDtoItems[_auctionID].Autobidder != address(0) && auctionIDtoItems[_auctionID].Autobidder != _bidder){
    ...
    function autobid (uint _auctionID) public payable returns (bool){
        ...
        require (auctionIDtoItems[_auctionID].AuctionStatus == true, "The auction needs to be open!");
            ...
            uint _bidvalue = msg.value;
            uint _current = auctionIDtoItems[_auctionID].lastBid;
            uint lim = _current*101/100;
            uint existingautobid = auctionIDtoItems[_auctionID].autobidderlimit;
            uint existlimit = existingautobid*101/100;

            if(auctionIDtoItems[_auctionID].Autobidder != address(0)){
                if(_bidvalue > existlimit){
                    biddingcalc(payable(msg.sender), _auctionID, lim);
                    address payable _newadd = payable(address(auctionIDtoItems[_auctionID].Autobidder));
                    payable(_newadd).transfer(auctionIDtoItems[_auctionID].autobidderlimit);
   ...
   function finishAuction (uint _auctionID) public returns (bool) {
        ...
        require (auctionIDtoItems[_auctionID].AuctionStatus == true, "The auction needs to be open!");
        require (block.timestamp >= auctionIDtoItems[_auctionID].startTime + 3 days, "Cannot be run before 3 days!");

        if(auctionIDtoItems[_auctionID].numberofbids >= 2){
        if(auctionIDtoItems[_auctionID].nftType == false){
        (bool success, ) = (auctionIDtoItems[_auctionID].nftContract).call(abi.encodeWithSignature("transferFrom(address,address,uint256)", payable(address(this)), payable(auctionIDtoItems[_auctionID].lastBidder), (auctionIDtoItems[_auctionID].tokenID)));
        require(success);
        }
        else{
        (bool success, ) = (auctionIDtoItems[_auctionID].nftContract).call(abi.encodeWithSignature("safeTransferFrom(address,address,uint256,uint256,bytes)", address(this), (auctionIDtoItems[_auctionID].lastBidder), (auctionIDtoItems[_auctionID].tokenID), (auctionIDtoItems[_auctionID].amountNFT1155), (auctionIDtoItems[_auctionID].data)));
        require(success);
        }
        address payable _newadd = payable(address(auctionIDtoItems[_auctionID].seller));
        uint _comm = (auctionIDtoItems[_auctionID].lastBid)*Auctioncomissionpercent/100;
        treasury += _comm;
        payable(_newadd).transfer((auctionIDtoItems[_auctionID].lastBid) - _comm);
        auctionIDtoItems[_auctionID].buyer = payable(auctionIDtoItems[_auctionID].lastBidder);
        if(auctionIDtoItems[_auctionID].Autobidder != address(0)){
            address payable _newadd2 = payable(address(auctionIDtoItems[_auctionID].Autobidder));
            payable(_newadd2).transfer((auctionIDtoItems[_auctionID].autobidderlimit) - (auctionIDtoItems[_auctionID].lastBid));
        }
   ...
   function cancelAuction (uint _auctionID) public returns (bool){
        ...
        if(auctionIDtoItems[_auctionID].numberofbids >= 1 && (auctionIDtoItems[_auctionID].Autobidder != address(0))){
            address payable _newadd2 = payable(address(auctionIDtoItems[_auctionID].Autobidder));
            payable(_newadd2).transfer((auctionIDtoItems[_auctionID].autobidderlimit));
        }
        ...
        if(auctionIDtoItems[_auctionID].nftType == false){
        (bool success, ) = (auctionIDtoItems[_auctionID].nftContract).call(abi.encodeWithSignature("transferFrom(address,address,uint256)", payable(address(this)), payable(auctionIDtoItems[_auctionID].seller), (auctionIDtoItems[_auctionID].tokenID)));
        require(success);
        }
        else{
        (bool success, ) = (auctionIDtoItems[_auctionID].nftContract).call(abi.encodeWithSignature("safeTransferFrom(address,address,uint256,uint256,bytes)", address(this), (auctionIDtoItems[_auctionID].seller), (auctionIDtoItems[_auctionID].tokenID), (auctionIDtoItems[_auctionID].amountNFT1155), (auctionIDtoItems[_auctionID].data)));
        require(success);
        }
        auctionIDtoItems[_auctionID].AuctionStatus = false;
        return true;
